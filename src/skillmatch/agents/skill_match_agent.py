...
class SkillMatchAgent:
                    ...
                    "skill_gaps": [
                        {
                            ...
                            "current": gap.current_level.value if gap.current_level else "None",
    ...
    async def calculate_match_score(
        ...
        try:
            ...
            match_score = self.skill_matcher.calculate_match_score(user_profile, opportunity)
    ...
    async def identify_skill_gaps(self, user_profile_json: str, target_role: str = "") -> str:
        """
        ...
        try:
            ...
            if target_role:
                ...
                for opp in self.opportunities_db.get_active_opportunities():
                    if target_role.lower() in opp.title.lower() or target_role.lower() in opp.description.lower():
                        relevant_opportunities.append(opp)
            else:
                # Use all active opportunities
                relevant_opportunities = self.opportunities_db.get_active_opportunities()
            
            # Collect skill gaps across opportunities
            all_gaps = {}
            for opp in relevant_opportunities[:10]:  # Limit to top 10 for analysis
                match_score = self.skill_matcher.calculate_match_score(user_profile, opp)
                for gap in match_score.skill_gaps:
            ...
            # Sort by importance
            sorted_gaps = sorted(all_gaps.values(), key=lambda g: g.importance, reverse=True)
                ...
                "skill_gaps": [
                    {
                        ...
                        "required_level": gap.required_level.value,
                        "importance": gap.importance,
                        "priority": "High" if gap.importance > 0.7 else "Medium" if gap.importance > 0.4 else "Low"
                    }
            ...
            for gap in gaps_data.get("skill_gaps", []):
                ...
                recommendations.append({
                    ...
                    "learning_opportunities": [
                        {
                            "title": opp.title,
                            "provider": opp.learning_info.provider if opp.learning_info else "N/A",
                            "duration": opp.learning_info.duration if opp.learning_info else opp.duration,
                            "cost": opp.learning_info.cost if opp.learning_info else 0,
                            "certification": opp.learning_info.certification if opp.learning_info else False
                        }
                        for opp in learning_opps[:3]  # Top 3 learning opportunities
                    ],
                    "general_advice": self._get_skill_learning_advice(skill_name, current_level, required_level)
                })
    ...
    async def analyze_user_strengths(self, user_profile_json: str) -> str:
        ...
        try:
            ...
            for category in strengths:
                if strengths[category]["total_skills"] > 0:
                    strengths[category]["avg_experience"] = round(
                        strengths[category]["avg_experience"] / strengths[category]["total_skills"], 1
                    )
            ...
            return json.dumps({
                "status": "error",
                "message": f"Error analyzing strengths: {str(e)}"
            })
        ...
        try:
            ...
            for opp in self.opportunities_db.get_opportunities_by_type("learning"):
                ...
                for req_skill in opp.get_all_skills():
                    if any(skill in req_skill.skill_name.lower() for skill in skills_list):
                        teaches_skill = True
                        break
                ...
                if opp.learning_info and opp.learning_info.cost and opp.learning_info.cost > max_cost:
                    continue
                
                # Check difficulty level
                if difficulty_level != "any" and opp.learning_info:
                    if difficulty_level.lower() not in opp.learning_info.difficulty.lower():
                        continue
                ...
                learning_opps.append({
                    ...
                    "duration": opp.learning_info.duration if opp.learning_info else opp.duration,
                    "difficulty": opp.learning_info.difficulty if opp.learning_info else "Not specified",
                    "cost": opp.learning_info.cost if opp.learning_info else 0,
                    "certification": opp.learning_info.certification if opp.learning_info else False,
                    "skills_taught": [skill.skill_name for skill in opp.get_all_skills()],
                    "prerequisites": opp.learning_info.prerequisites if opp.learning_info else []
                })
    ...
    def _generate_recommendations(self, match_score: MatchScore) -> List[str]:
        ...
        if match_score.overall_score >= 0.8:
            recommendations.append("This is an excellent match! Consider applying immediately.")
        elif match_score.overall_score >= 0.6:
            recommendations.append("This is a good match with some areas for improvement.")
        else:
            recommendations.append("This opportunity might be challenging but could offer good growth.")
        
        if match_score.skill_gaps:
            high_priority_gaps = [gap for gap in match_score.skill_gaps if gap.importance > 0.7]
            if high_priority_gaps:
                skills_list = ", ".join([gap.skill_name for gap in high_priority_gaps[:3]])
                recommendations.append(f"Focus on developing: {skills_list}")
        
        if match_score.strengths:
            recommendations.append(f"Highlight your strengths in: {', '.join(match_score.strengths[:3])}")
        
        return recommendations
    
    def _get_skill_learning_advice(self, skill_name: str, current_level: str, target_level: str) -> str:
        ...
        return advice_map.get(skill_name.lower(), 
