...
class AIJobMatcher:
    ...
    def __init__(self):
        ...
        # OpenAI API model
        self.openai_models = [
            "gpt-5-mini",  # Single model for all tasks
        ]
    ...
    def _setup_ai_clients(self):
        ...
        try:
            ...
            if openai_key:
                ...
                self.current_model = self.openai_models[0]  # Use gpt-5-mini
        ...
        try:
            ...
            if github_token:
                ...
                if not self.openai_client:
                    ...
                    logger.info(
                        f"✅ GitHub AI client initialized as primary with {self.current_model}"
                    )
                else:
    ...
    async def analyze_profile_comprehensive(
    ...
    async def intelligent_job_matching(
        self, profile_data: Dict[str, Any], opportunities: List[Dict[str, Any]]
    ) -> List[EnhancedMatch]:
        """
            ...
            for match_data in analysis_result.get("matches", []):
                ...
                opp = next(
                    (
                        ...
                        if o.get("opportunity_id") == match_data.get("opportunity_id")
                    ),
                    None,
                )
                if opp:
                    insights = MatchInsight(
                        ...
                        career_trajectory=match_data.get(
                    ...
                    enhanced_match = EnhancedMatch(
                        ...
                        title=opp.get("title", ""),
                        company=opp.get("company", {}).get("name", "Unknown")
                        if isinstance(opp.get("company"), dict)
                        else str(opp.get("company", "Unknown")),
            ...
            if self.openai_client:
                for model in self.openai_models:
                    try:
                        ...
                        logger.warning(
                            f"OpenAI model {model} failed (attempt {attempt + 1}): {e}"
                        )
                        continue
            ...
            if self.github_client:
                for model in self.github_models:
                    try:
                        ...
                        logger.warning(
                            f"GitHub model {model} failed (attempt {attempt + 1}): {e}"
                        )
                        continue
    ...
    def _build_profile_summary(
        self, profile_data: Dict[str, Any], profile_analysis: Dict[str, Any]
    ) -> str:
    ...
    def _build_opportunities_context(self, opportunities: List[Dict[str, Any]]) -> str:
        ...
        for opp in opportunities:
            ...
            req_skill_names = [
                skill.get("skill_name", "") for skill in required_skills[:5]
            ]
    ...
    def _fallback_matching(
        ...
        for opp in opportunities[:10]:  # Limit to top 10 for fallback
            ...
            skill_matches = sum(
                ...
                if any(
                    req_skill.get("skill_name", "").lower() in user_skill
...
async def get_ai_job_matches(
    profile_data: Dict[str, Any], opportunities: List[Dict[str, Any]]
) -> List[Dict[str, Any]]:
    """
    ...
    """
    try:
        enhanced_matches = await ai_job_matcher.intelligent_job_matching(
...
if __name__ == "__main__":
    ...
    async def test_ai_matcher():
        ...
        for result in results:
            print(
                f"- {result['title']} at {result['company']}: {result['overall_score']}% match"
            )
