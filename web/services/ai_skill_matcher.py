...
class AISkillMatcher:
    ...
    def initialize_openai(self):
        ...
        try:
            ...
            if openai_key and openai_key.startswith("sk-"):
                ...
                logger.info("✅ Initialized AI Skill Matcher with OpenAI (gpt-5-mini)")
            else:
                logger.warning("⚠️ No valid AI API keys found, using fallback matching")
                logger.info("💡 To enable AI: Set OPENAI_API_KEY (sk-...)")
    ...
    def categorize_skills(self, skills: List[str]) -> List[SkillCategory]:
        ...
        try:
            ...
            if "unauthorized" in error_msg or "401" in error_msg:
                logger.error(
                    f"❌ AI authorization failed: Invalid API key. Please check your GitHub token or OpenAI key."
                )
    ...
    def find_skill_matches(
        self, user_skills: List[str], job_description: str, job_title: str
    ) -> List[SkillMatch]:
        """
        ...
        try:
            ...
            if "unauthorized" in error_msg or "401" in error_msg:
                logger.error(
                    f"❌ AI authorization failed: Invalid API key. Check GITHUB_TOKEN or OPENAI_API_KEY"
                )
            elif "too many requests" in error_msg or "429" in error_msg:
                logger.warning(
                    f"⚠️ AI rate limit exceeded. Using enhanced fallback matching."
                )
            else:
                logger.error(f"❌ AI skill matching failed: {e}")
            return self._fallback_matching(user_skills, job_description, job_title)
    ...
    async def extract_job_skills(
        ...
        try:
            ...
            normalized_skills = list(
                set([skill.lower().strip() for skill in all_skills if skill])
            )
            ...
            if "unauthorized" in error_msg or "401" in error_msg:
                ...
                logger.warning(
                    f"⚠️ AI rate limit hit. Using enhanced skill extraction fallback."
                )
            else:
                logger.error(f"❌ AI skill extraction failed: {e}")
            return self._fallback_skill_extraction(job_title, job_description)
    ...
    def _fallback_categorization(self, skills: List[str]) -> List[SkillCategory]:
        ...
        health_skills = [
            skill
            for skill in skills
            if any(keyword in skill.lower() for keyword in healthcare_keywords)
        ]
        if health_skills:
            categories.append(
                SkillCategory(
        ...
        data_skills = [
            skill
            for skill in skills
            if any(keyword in skill.lower() for keyword in data_keywords)
        ]
        if data_skills:
            categories.append(
                SkillCategory(
    ...
    def _fallback_skill_extraction(
        self, job_title: str, job_description: str
    ) -> List[str]:
...
async def find_skill_matches_ai(
