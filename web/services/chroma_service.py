...
class ChromaDBService:
    ...
    def add_resume_to_vector_db(
        self, profile_id: str, pdf_path: str, metadata: Dict[str, Any] = None
    ) -> bool:
        ...
        try:
            ...
            print(
                f"✅ Added {len(chunks)} chunks to resume collection for {profile_id}"
            )
            return True
    ...
    def search_similar_jobs(
        self, resume_text: str, n_results: int = 10
    ) -> List[Dict[str, Any]]:
        """Find jobs similar to resume content using vector search"""
        try:
            logger.debug(f"🔍 Searching for jobs similar to resume content...")
            ...
            logger.debug(f"Vector search completed in {elapsed_ms:.2f}ms")
    ...
    def batch_search_similar_jobs(
        self, resume_texts: List[str], n_results: int = 10, batch_size: int = 5
    ) -> List[List[Dict[str, Any]]]:
        """Find jobs similar to multiple resumes (batch operation).

        Optimized for processing multiple profiles efficiently.
        ...
        Returns:
            List of lists of similar jobs
        """
        try:
            logger.info(f"🔍 Batch searching for {len(resume_texts)} resumes...")
            ...
            for batch_start in range(0, len(resume_texts), batch_size):
                batch_end = min(batch_start + batch_size, len(resume_texts))
                batch = resume_texts[batch_start:batch_end]
                ...
                try:
                    ...
                    for query_idx, metadatas in enumerate(results["metadatas"]):
                        similar_jobs = []
                        seen_jobs = set()

                        for res_idx, metadata in enumerate(metadatas):
                            job_id = metadata.get("job_id")
                            if job_id not in seen_jobs:
                                similarity_score = (
                                    1 - results["distances"][query_idx][res_idx]
                                )
                                similar_jobs.append(
                                    {
                                        "job_id": job_id,
                        ...
                        all_results.append(similar_jobs)
            ...
            elapsed_ms = (time.perf_counter() - start_time) * 1000
            logger.info(f"✅ Batch search completed in {elapsed_ms:.2f}ms")
    ...
    def get_resume_insights(self, profile_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            results = self.resume_collection.get(
                where={"profile_id": profile_id}, include=["metadatas", "documents"]
            )

            if not results["ids"]:
                return {
                    "status": "no_resume",
                    "message": "No resume found in vector database",
                }

            total_chunks = len(results["ids"])
            total_text_length = sum(
                meta.get("text_length", 0) for meta in results["metadatas"]
            )
    ...
    def initialize_existing_resumes(self, uploads_dir: str = "uploads/resumes"):
        ...
        try:
            ...
            print(
                f"🎉 Initialization complete. Processed {len(resume_files)} resume files."
            )
