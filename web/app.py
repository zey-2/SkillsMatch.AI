        ...
        for job in jobs_list[:15]:  # Reduced to 15 jobs for faster AI analysis
            job_summary = {
                ...
                "description": job.get("job_description", job.get("description", ""))[
                    :300
                ],  # More description context
                "required_skills": job.get(
                    "job_skill_set", job.get("required_skills", [])
                )[:10],  # Limit skills for token efficiency
            }
            ...
            try:
                response = client.chat.completions.create(
                    ...
                    messages=[
                        {
                            "role": "system",
                            "content": "You are an expert career matching AI that provides precise job matching analysis. Always return valid JSON responses with comprehensive scoring.",
                        },
                ...
                if (
                    ...
                    or "insufficient" in str(model_error).lower()
                ):
                    print(
                        "💡 OpenAI quota exceeded - generating enhanced mock response"
                    )
...
def _generate_enhanced_mock_ai_response(profile_data, jobs_list):
    ...
    try:
            ...
            try:
                ...
                def run_ai_matching():
                    ...
                    return loop.run_until_complete(
                        find_enhanced_matches(profile_data, jobs_list, 20)
                    )
                ...
                for match in enhanced_matches:
                    ...
                    job = next(
                        (j for j in jobs_list if j.get("id") == match.job_id), None
                    )
                    if job:
                        scored_jobs.append(
                            {
                                ...
                                "skill_matches": [
                                    m.user_skill for m in match.skill_matches
                                ],
                                "ai_analysis": {
        ...
        for job in jobs_list[:100]:  # Analyze more jobs for better matches
            ...
            job_skills = []
            all_job_text = (
                f"{job_keywords} {job_title} {job_category} {job_description}".lower()
            )
            ...
            is_healthcare_job = any(
                indicator in all_job_text for indicator in healthcare_indicators
            )
            if is_healthcare_job:
                job_skills.extend(["healthcare", "medical", "patient care", "clinical"])
                print(
                    f"🏥 Detected healthcare job: {job_title} - added healthcare skills"
                )
            ...
            for job_skill in job_skills_lower:
                ...
                for user_skill in user_skills:
                    ...
                    if (
                        job_skill_clean == user_skill_clean
                        or job_skill_clean in user_skill_clean
                        or user_skill_clean in job_skill_clean
                    ):
                ...
                if not skill_matched:
                    for user_skill in user_skills:
                        ...
                        for skill_category, synonyms in skill_synonyms.items():
                            if (
                                user_skill_clean in synonyms
                                and job_skill_clean in synonyms
                            ):
                                matched_skills.append(job_skill)
                                print(
            ...
            skill_coverage = (
                len(matched_skills) / max(len(job_skills_lower), 1)
                if job_skills_lower
                else 0
            )
            ...
            for rule_name, rule in exclusion_rules.items():
                user_has_indicators = any(
                    indicator in user_context.lower()
                    for indicator in rule["user_indicators"]
                )
                job_is_excluded_type = any(
                    excluded_type in job_context.lower()
                    for excluded_type in rule["excluded_job_types"]
                )
                        ...
                        excluded_hr_jobs_ai.append(
                            {
                                ...
                                "hr_keywords_found": [
                                    excluded_type
                                    for excluded_type in rule["excluded_job_types"]
                                    if excluded_type in job_context.lower()
                                ],
                            }
                        )

                    print(
                        f"🚫 AI EXCLUDING job {job.get('job_id', 'unknown')}: {job_title} - {rule_name} exclusion rule triggered"
                    )
                    should_exclude = True
            ...
            for industry, keywords in industry_keywords.items():
                user_industry_match = sum(
                    1 for kw in keywords if kw in user_context.lower()
                ) / len(keywords)
                job_industry_match = sum(
                    1 for kw in keywords if kw in job_context.lower()
                ) / len(keywords)
                ...
                if user_industry_match > 0.3 and job_industry_match > 0.3:
                    ...
                    industry_score = max(
                        industry_score,
                        min(user_industry_match, job_industry_match) * 0.95,
                    )
                elif user_industry_match > 0.1 and job_industry_match > 0.1:
                    # Moderate industry alignment
                    industry_score = max(
                        industry_score,
                        min(user_industry_match, job_industry_match) * 0.6,
                    )
            ...
            growth_score = 0.6 + (
                sum(
                    1 for indicator in growth_indicators if indicator in job_description
                )
                * 0.1
            )
            ...
            meets_basic_threshold = (
                match_percentage >= 15
            )  # Very low threshold to show more matches
            ...
            if has_meaningful_skills and meets_basic_threshold:
                ...
                skills_only_percentage = (
                    (len(matched_skills) / len(job_skills_lower)) * 100
                    if job_skills_lower
                    else 0
                )
                ...
                scored_jobs.append(
                    {
                        ...
                        "skills_only_percentage": round(
                            skills_only_percentage, 1
                        ),  # Add pure skills match
        ...
        if excluded_hr_jobs_ai:
            ...
            for i, excluded_job in enumerate(excluded_hr_jobs_ai, 1):
                ...
                print(
                    f"    HR Keywords: {', '.join(excluded_job['hr_keywords_found'])}"
                )
                print()
            print("=" * 60)
            print(
...
def _generate_match_reasoning(
    ...
    if skill_score > 0.7:
        reasons.append(
            f"Excellent skills alignment with {len(matched_skills)} key competencies"
        )
    elif skill_score > 0.5:
        reasons.append(f"Strong skills match in {len(matched_skills)} areas")
    else:
        reasons.append(
            f"Growing potential with {len(matched_skills)} transferable skills"
        )
    ...
    return (
        f"{match_percentage:.0f}% match featuring "
        + ", ".join(reasons)
        + ". This role offers excellent career advancement potential."
    )
    ...
    try:
        ...
        if current_role:
            ...
            context_details.append(
                f"{years_exp} years experience as {current_role.get('position')} at {current_role.get('company')}"
            )
        if highest_education:
            context_details.append(
                f"{highest_education.get('degree')} in {highest_education.get('field_of_study')} from {highest_education.get('institution')}"
            )
        if resume_content:
        ...
        if current_role:
            ...
            if current_role.get("description"):
                work_description += (
                    f", specializing in {current_role.get('description')}"
                )
            ...
            try:
                response = client.chat.completions.create(
                    ...
                    messages=[
                        {
                            "role": "system",
                            "content": "You are an expert career writer who creates compelling professional summaries. Write engaging, accomplished-sounding summaries that showcase the person's expertise and potential. Use dynamic language and focus on achievements and capabilities.",
                        },
                        {"role": "user", "content": prompt},
                    ],
                    max_tokens=120,
...
app.config["SECRET_KEY"] = os.environ.get(
    "SECRET_KEY", "skillmatch-production-key-change-me"
)
...
app.config["PREFERRED_URL_SCHEME"] = (
    "http" if os.environ.get("FLASK_ENV") != "production" else "https"
)
...
socketio = SocketIO(
    ...
    async_mode="gevent",  # Using gevent for better httpx compatibility
...
register_socket_handlers(socketio, load_config)
...
def index():
    ...
    if data_loader:
        ...
        if (
            hasattr(data_loader, "opportunities_data")
            and data_loader.opportunities_data
        ):
            stats["total_opportunities"] = len(
                data_loader.opportunities_data.get("opportunities", [])
            )
                ...
                if job.job_category:
                    ...
                    try:
                        ...
                        # Handle JSON string format like '["F&B"]'
                            ...
                            job.job_category, str
                        ) and job.job_category.startswith("["):
                            import json
                        ...
                        # Handle plain string categories
                            ...
                            isinstance(job.job_category, str)
                            and job.job_category.strip()
                        ):
                            category_counts[job.job_category.strip()] = (
                                category_counts.get(job.job_category.strip(), 0) + 1
                            )
    ...
    if scraped_data_dir.exists():
        ...
        if scrape_files:
            ...
            stats["last_scrape"] = datetime.fromtimestamp(
                latest_scrape.stat().st_mtime
            ).strftime("%Y-%m-%d %H:%M")
            ...
            "data": [
                {
                    ...
                    "marker": {
                        "color": bar_colors,
                        "line": {"color": bar_border_colors, "width": 2},
                        "opacity": 0.9,
                    },
...
def dashboard():
    ...
    try:
        ...
        try:
            ...
            with db_config.session_scope() as session:
                # Count total jobs
                dashboard_stats["total_jobs"] = (
                    session.query(Job).filter(Job.is_active == True).count()
                )
                            ...
                            for category in job.job_category:
                                if category and isinstance(category, (str, dict)):
                                    cat_name = (
                                        ...
                                        else category.get("name", str(category))
                                    )
                                    if cat_name:
                ...
                # Sort categories by count and take top 10
                dashboard_stats["job_categories"] = dict(
            ...
            profiles_dir = Path(__file__).parent.parent / "profiles"
            if profiles_dir.exists():
                dashboard_stats["total_profiles"] = len(
            ...
            def generate_gradient_colors_dashboard(num_categories):
                ...
                for i in range(num_categories):
                    ...
                    border_colors.append(
                        f"rgba({border_r}, {border_g}, {border_b}, 1.0)"
                    )

                return colors, border_colors

            # Generate gradient colors for all categories
            bar_colors, bar_border_colors = generate_gradient_colors_dashboard(
                len(categories)
            )
    ...
    try:
        ...
        for profile_data in profiles_data:
            ...
            profile_obj = {
                ...
                "created_at": parse_datetime(profile_data.get("created_at"))
                or datetime.now(),
                "modified": parse_datetime(profile_data.get("updated_at"))
                or datetime.now(),
            }
...
def create_profile():
    ...
    if data_loader and data_loader.skills_data:
        for category, category_data in data_loader.skills_data.get(
            "skills", {}
        ).items():
...
def save_profile():
    ...
    try:
        ...
        profile_data = {
            ...
            "education": [],
            "preferences": {
                "work_types": [request.form.get("work_type")]
                if request.form.get("work_type")
                else [],
                "locations": [request.form.get("location")]
                if request.form.get("location")
                else [],
                "industries": json.loads(request.form.get("industries", "[]"))
                if request.form.get("industries")
                else [],
                "salary_min": int(float(request.form.get("salary_min")))
                if request.form.get("salary_min")
                and request.form.get("salary_min").strip()
                else None,
                "salary_max": int(float(request.form.get("salary_max")))
                if request.form.get("salary_max")
                and request.form.get("salary_max").strip()
                else None,
            ...
            if data_loader and data_loader.skills_data:
                for category, category_data in data_loader.skills_data.get(
                    ...
                    if skill_id in category_data.get("skills", {}):
                        skill_name = category_data["skills"][skill_id].get(
                            "name", skill_id
                        )
            ...
            profile_data["work_experience"].append(
                {
                    ...
                    "years": int(float(request.form.get("experience_years")))
                    if request.form.get("experience_years")
                    and request.form.get("experience_years").strip()
                    else 0,
            ...
            profile_data["education"].append(
                {
                    ...
                    "graduation_year": int(float(request.form.get("graduation_year")))
                    if request.form.get("graduation_year")
                    and request.form.get("graduation_year").strip()
        ...
        if "resume" in request.files:
            ...
            if (
                ...
                resume_filename = (
                    f"{profile_data['name'].lower().replace(' ', '_')}_resume.pdf"
                )
        ...
        if success:
            if is_editing:
                ...
                flash(
                    f"🎉 Profile '{profile_data['name']}' created successfully!",
                    "success",
                )
        else:
...
def view_profile(profile_id):
    ...
    try:
        ...
        return render_template(
            "view_profile.html", profile=profile_data, profile_id=profile_id
        )
...
def download_resume(profile_id):
    ...
    try:
        ...
        print(
            f"📥 Downloading resume: {resume_filename} for {profile_data.get('name', 'Unknown')}"
        )
...
def debug_test():
    ...
    return send_from_directory(
        os.path.dirname(os.path.dirname(__file__)), "debug_test.html"
    )
    ...
    try:
        ...
        try:
            ...
            with db_config.session_scope() as session:
                ...
                for job in jobs:
                    ...
                    employment_types = (
                        job.employment_type if job.employment_type else []
                    )
                ...
                if ai_results and "top_matches" in ai_results:
                    ...
                    for ai_match in ai_results["top_matches"]:
                        ...
                        if original_job:
                            ...
                            salary_info = ""
                            if original_job.get("min_salary") and original_job.get(
                                "max_salary"
                            ):
                                ...
                                salary_info = (
                                    f"Up to SGD {original_job['max_salary']:,}"
                                )
                            ...
                            final_matches.append(
                                {
                                    ...
                                    "location": original_job.get(
                                        "location",
                                        profile_data.get("location", "Singapore"),
                                    ),
                                    "category": original_job["category"],
                                    "description": original_job["job_description"][:400]
                                    + "..."
                                    if len(original_job.get("job_description", ""))
                                    > 400
                                    else original_job.get("job_description", ""),
                                    "required_skills": [],  # Will be extracted from job description by AI
                                    "position_level": original_job.get(
                                        "position_level", ""
                                    ),
                                    "employment_type": original_job.get(
                                        "employment_type", []
                                    ),
                                    "work_arrangement": original_job.get(
                                        "work_arrangement", ""
                                    ),
                                    "salary_range": salary_info,
                                    "match_score": ai_match["comprehensive_score"],
                                    "match_percentage": ai_match.get(
                                        "overall_match_score",
                                        ai_match.get("match_percentage", 0),
                                    ),
                                    "skills_only_percentage": ai_match.get(
                                        "skills_only_percentage", 0
                                    ),
                                    "matched_skills": ai_match.get(
                                        "matched_skills", []
                                    ),
                                    "missing_skills": ai_match.get("skill_gaps", []),
                                    "skills_matched_count": len(
                                        ai_match.get("matched_skills", [])
                                    ),
                                    "total_required_skills": len(
                                        ai_match.get("matched_skills", [])
                                        + ai_match.get("skill_gaps", [])
                                    ),
                                    "recommendation_reason": ai_match[
                                        "recommendation_reason"
                                    ],
                                    "growth_opportunities": ai_match.get(
                                        "growth_opportunities", ""
                                    ),
                                    "source": "ai_enhanced",
                                    "skill_match_score": ai_match.get(
                                        "skill_match_score", 0
                                    ),
                                    "industry_match_score": ai_match.get(
                                        "industry_match_score", 0
                                    ),
                                    "education_match_score": ai_match.get(
                                        "education_match_score", 0
                                    ),
                                    "location_match_score": ai_match.get(
                                        "location_match_score", 0
                                    ),
                                    "career_growth_score": ai_match.get(
                    ...
                    matching_info.update(
                        {
                            "ai_analysis_summary": ai_results.get(
                                "analysis_summary", ""
                            ),
        ...
        if not final_matches and all_available_jobs:
            ...
            for job in all_available_jobs[:150]:  # Analyze more jobs
                ...
                all_job_text = (
                    f"{job_keywords} {job_title} {job_category} {job_description}"
                )
                    ...
                    excluded_hr_jobs.append(
                        {
                            ...
                            "hr_keywords_found": [
                                ...
                                if hr in job_context
                            ],
                        }
                    )
                    print(
                        f"🚫 TRADITIONAL: Excluding HR job {job.get('job_id', 'unknown')}: {job_title}"
                    )
                ...
                for job_skill in job_skills_lower:
                    ...
                    for user_skill in user_skills:
                        ...
                        # Partial match
                            ...
                            score = max(
                                len(user_skill) / len(job_skill),
                                len(job_skill) / len(user_skill),
                            )
                ...
                if skill_relevance_scores:
                    avg_relevance = sum(skill_relevance_scores) / len(
                        skill_relevance_scores
                    )
                    coverage = (
                        len(matched_skills) / max(len(job_skills_lower), 1)
                    ...
                    traditional_matches.append(
                        {
                            ...
                            "description": job["job_description"][:300] + "..."
                            ...
                            "missing_skills": [
                                ...
                                if s not in [m.lower() for m in matched_skills]
                            ][:10],
                            "skills_matched_count": len(matched_skills),
                            "total_required_skills": len(job_skills_lower),
                            "recommendation_reason": _create_simple_match_reason(
                                match_percentage, len(matched_skills), job["category"]
                            ),
                            "source": "traditional",
                            "skill_match_score": skill_match_score,
                            "category_match_score": 0.2,
                            "user_skill_coverage": len(matched_skills)
                            / max(len(user_skills), 1),
                        }
                    )
        ...
        matching_info.update(
            {
                ...
                "top_match_score": final_matches[0]["match_percentage"]
                if final_matches
                else 0,
        ...
        return jsonify({**matching_info, "matches": final_matches})
...
def api_match_efficient():
    ...
    try:
        ...
        matching_result = vector_job_matcher.match_jobs_efficiently(
            profile_data=profile_data, limit=limit
        )
        ...
        for match in matching_result.get("matches", []):
            formatted_match = {
                ...
                "match_percentage": 90
                - (match.get("match_rank", 1) * 5),  # Simulated scores
            ...
            with db_config.session_scope() as session:
                ...
                if job:
                    job_data = {
                        ...
                        or "Singapore Companies",  # Use actual company name
                        "company_name": job.company_name,
                        "category": job.job_category
                        if isinstance(job.job_category, str)
                        else ", ".join(job.job_category)
                        if job.job_category
                        else "General",
                    ...
                    job_text = (
                        f"{job.keywords or ''} {job.job_description or ''}".lower()
                    )
                    common_skills = [
                        "python",
                        "java",
                    ...
                    for job_skill in job_skills_lower:
                        ...
                        for user_skill in user_skills:
                            ...
                            # Partial match (contains)
                                ...
                                if (
                                    len(user_skill) >= 3 and len(job_skill) >= 3
                                ):  # Avoid short word false matches
                                    match_ratio = min(
                                        len(user_skill), len(job_skill)
                                    ) / max(len(user_skill), len(job_skill))
                                    relevance_score = 0.5 + (
                                        match_ratio * 0.2
                                    )  # 0.5-0.7 range
                    ...
                    if user_category and user_category in job_category_lower:
                        ...
                        any(
                            keyword in job_category_lower
                            for keyword in user_category.split()
                        )
                        if user_category
                        else False
                    ):
...
def fetch_jobs_from_api():
    ...
    try:
        ...
        if not all_jobs_data:
            return jsonify(
                {"success": False, "error": "No job data found in API response"}
            ), 400
            ...
            for job_item in jobs_data:
                try:
                    ...
                    if job_data.get("EmploymentType"):
                        employment_types = [
                            et.get("caption", "")
                            for et in job_data["EmploymentType"]
                            if et.get("caption")
                        ]
                    ...
                    if job_data.get("JobCategory"):
                        job_categories = [
                            jc.get("caption", "")
                            for jc in job_data["JobCategory"]
                            if jc.get("caption")
                        ]
                    ...
                    if job_data.get("id_Job_NearestMRTStation"):
                        mrt_stations = [
                            mrt.get("caption", "")
                            for mrt in job_data["id_Job_NearestMRTStation"]
                            if mrt.get("caption")
                        ]
                    ...
                    if job_data.get("id_Job_TimingShift"):
                        timing_shifts = [
                            ts.get("caption", "")
                            for ts in job_data["id_Job_TimingShift"]
                            if ts.get("caption")
                        ]

                    # Extract location data
                    google_place = (
                        company_data.get("GooglePlace", {}) if company_data else {}
                    )

                    # Create unique job_id if not provided
                    job_id = (
                        job_data.get("id")
                        or f"job_{job_data.get('company_sid', '')}_{jobs_added}"
                    )
                    ...
                    try:
                        job = Job(
                            ...
                            activation_date=parse_api_datetime(
                                job_data.get("activation_date")
                            ),
                            expiration_date=parse_api_datetime(
                            ...
                            # Job details
                            position_level=job_data.get("id_Job_PositionLevel", {}).get(
                                "caption", ""
                            )
                            if job_data.get("id_Job_PositionLevel")
                            else "",
                            min_years_experience=job_data.get(
                                "MinimumYearsofExperience", {}
                            ).get("caption", "")
                            if job_data.get("MinimumYearsofExperience")
                            else "",
                            min_education_level=job_data.get(
                                "MinimumEducationLevel", {}
                            ).get("caption", "")
                            if job_data.get("MinimumEducationLevel")
                            else "",
                            ...
                            salary_interval=job_data.get("id_Job_Interval", {}).get(
                                "caption", ""
                            )
                            if job_data.get("id_Job_Interval")
                            else "",
                            currency=job_data.get("id_Job_Currency", {}).get(
                                "caption", ""
                            )
                            if job_data.get("id_Job_Currency")
                            else "",
                            employment_type=employment_types,
                            work_arrangement=job_data.get(
                                "id_Job_WorkArrangement", {}
                            ).get("caption", "")
                            if job_data.get("id_Job_WorkArrangement")
                            else "",
                            ...
                            company_description=company_data.get(
                                "CompanyDescription", ""
                            )
                            if company_data
                            else "",
                            company_uen=company_data.get("id__CompanyUEN", "")
                            if company_data
                            else "",
                            company_country_code=company_data.get(
                                "id__Companycountrycode", ""
                            )
                            if company_data
                            else "",
                            # Location
...
if __name__ == "__main__":
    ...
    if os.environ.get("FLASK_ENV") == "production":
        ...
        socketio.run(
            app, debug=True, host="0.0.0.0", port=port, allow_unsafe_werkzeug=True
        )
